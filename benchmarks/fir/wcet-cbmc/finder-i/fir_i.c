...
#define incrementTime(lTime) (timeElapsed = (timeElapsed + ( lTime )))
                    ...
                    long scale)
{
  ...
  for(i = 0 ; (incrementTime (21)), (i < in_len) ; i++) {					// BB 2
//    						incrementTime ( 20 );				// BB 18

    /* set up pointer for accumulation */
    data_ptr = in_ptr;				
    						incrementTime ( 52 );				// BB 3
    coef_ptr = coef_start;

    /* do accumulation and write result with scale factor */

						incrementTime ( 82 );				// BB 5 -  __mulsi3 function in binary
    acc = (long)(*coef_ptr++) * (*data_ptr--);	
    						incrementTime ( 44 );				// BB 6
    for(j = 1 ; (incrementTime (21)), (j < acc_length) ; j++)		{			// BB 7
	    					incrementTime ( 82 );				// BB 10 - __mulsi3 function in binary
	    					incrementTime ( 36 );				// BB 8
	    acc += (long)(*coef_ptr++) * (*data_ptr--);		
						incrementTime ( 62 );				// BB 12
    } 						incrementTime ( 1 );				// BB 7 - False branch
    								
						incrementTime ( 24 );				// BB 9
    *out++ = (int)(acc/scale);			incrementTime ( 737 );				// BB 11 - __divmodsi4 function in binary. TODO - model as loop

 						incrementTime ( 5 );				// BB 13 - Skipping one statement not included. TODO	
 						incrementTime ( 1 );				// BB 14 - Skipping one statement not included. TODO	
						incrementTime ( 35 );				// BB 15

    /* check for end case */
    if(in_ptr == in_end) {
						incrementTime ( 32 );				// BB 16
    ...
    /* if not at end, then check for startup, add to input pointer */
						...
						incrementTime ( 1 );				// BB 15 - False branch
						incrementTime ( 21 );				// BB 17
      if(acc_length < coef_len) {
      						incrementTime ( 20 );				// BB 19
	      acc_length++;
      }
      else
      {						incrementTime ( 1 );				// BB 17 - False branch - Else not present in original program, 
	      											// added for timing
      }
						incrementTime ( 10 );				// BB 20
      in_ptr++;      
    }
						incrementTime ( 20 );				// BB 18 - Iterator increment
  }						incrementTime ( 1 );				// BB 2 - False branch 
						incrementTime ( 35 );				// BB 4
}
...
void  CBMCDriver1()
{
    ...
    for (i=0;i<701;i++ )
    {
	    in_data [i] = nondet_long( );
    }
    fir_filter_int(in_data, output, 700, fir_int, 35, 285);
}


void CBMCDriver2(void)
{
unsigned int timeElapsedBefore = timeElapsed;
    long  output [720];
    long  local_in_data [701];
    fir_filter_int(local_in_data, output, 700, fir_int, 35, 285);
