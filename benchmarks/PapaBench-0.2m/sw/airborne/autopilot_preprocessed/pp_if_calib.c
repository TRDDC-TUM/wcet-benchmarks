...
void inflight_calib(bool_t mode_changed) {
  if (pprz_mode == 1) {
    if (inflight_calib_mode == 2) {
      static int16_t ir_pitch_neutral_init;
      if (mode_changed) {
        ir_pitch_neutral_init = ir_pitch_neutral;
        slider1_init = from_fbw.channels[5];
      }
      ir_pitch_neutral = (ir_pitch_neutral_init + (int16_t)(((float)(from_fbw.channels[5] - slider1_init)) * 60.000000 / (float)(600 * 16)));
    ...
    if (inflight_calib_mode == 2) {
      ...
      ir_roll_neutral = (ir_roll_neutral_init + (int16_t)(((float)(from_fbw.channels[6] - slider2_init)) * -60.000000 / (float)(600 * 16)));
  ...
  if (pprz_mode == 2) {
    if (inflight_calib_mode == 2) {
      ...
      course_pgain = (course_pgain_init + ((float)(from_fbw.channels[5] - slider1_init)) * 0.100000 / (float)(600 * 16));
    ...
    if (inflight_calib_mode == 2) {
      ...
      max_roll = (max_roll_init + ((float)(from_fbw.channels[6] - slider2_init)) * -0.200000 / (float)(600 * 16));
